(ns co.gaiwan.compass.css.colors)
...
(def open-color
  {:gray   ["#f8f9fa" "#f1f3f5" "#e9ecef" "#dee2e6" "#ced4da"
...
(def open-props-normalize-surface
  {:surface-1 "var(--surface-1)"
   :surface-2 "var(--surface-2)"
   :surface-3 "var(--surface-3)"
   :surface-4 "var(--surface-4)"})

(def girouette-tokens
  (merge
   (update-vals
    (into off-black-white
          (for [[k v] open-color
                [idx color] (map-indexed vector v) ]
            [(keyword (str (name k) "-" (inc idx)))
             color]))
    #(subs % 1))
